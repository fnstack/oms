...
type CreateProductInputValidator() =
    ...
    do
        ...
        base.RuleFor(fun input -> input.BrandId).NotEmpty().Must(fun value ->
            match value |> ObjectId.TryParse with
            | (true, _) -> true
            | false, _ -> false)
        |> ignore
        base.RuleFor(fun input -> input.CategoryId).NotEmpty().Must(fun x ->
            match x |> ObjectId.TryParse with
            | (true, _) -> true
            | false, _ -> false)
        |> ignore

let createProductInputValidator = new CreateProductInputValidator()

type EditProductInputValidator() =
    inherit AbstractValidator<EditProductInput>()
    do
        base.RuleFor(fun input -> input.Id).NotEmpty().Must(fun value ->
            match value |> ObjectId.TryParse with
            | (true, _) -> true
            | false, _ -> false)
        |> ignore
